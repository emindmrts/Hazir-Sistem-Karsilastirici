...
import {
    ArrowLeft, ExternalLink, ShieldCheck, Store, GitCompareArrows
} from "lucide-react"
...
function SpecRow({ icon, accent, label, value }: { icon: React.ReactNode; accent: string; label: string; value?: string }) {
    if (!value || value === "N/A") return null
    const [bgClass, textClass] = accent.split(" ")
    return (
        <div className="flex items-start gap-3 py-3 border-b border-border/40 last:border-0">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${bgClass} ${textClass}`}>
                {icon}
            </div>
...
function CompareButton({ product }: { product: Product }) {
    ...
    return (
        <Button
            variant="outline"
            className={`w-full rounded-full font-bold gap-2 transition-all ${
                inCompare
        ...
        >
            ...
            {inCompare ? "Karşılaştırmaya Eklendi" : isFull ? "Karşılaştırma Dolu (4/4)" : "Karşılaştırmaya Ekle"}
        </Button>
    )
}
...
export function DetailPage({ product, allProducts }: DetailPageProps) {
    ...
    // Strip spec label prefixes ("İşlemci:", "Ekran Kartı:") to avoid duplication
    const cleanCpu = (product.islemci || "").replace(/^(İşlemci|CPU)\s*[:\-]\s*/i, "").trim()
    const cleanGpu = (product.ekranKarti || "").replace(/^(Ekran Kartı|GPU)\s*[:\-]\s*/i, "").trim()
    const pageDesc = `${product.magaza} mağazasından ${product.sistemAdi} hazır sistem bilgisayarı. ${cleanCpu ? cleanCpu + " işlemcili" : ""} ${cleanGpu ? "ve " + cleanGpu + " ekran kartlı" : ""} bu sistemi ${product.fiyat.toLocaleString("tr-TR")} ₺ fiyatıyla inceleyin.`
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 xl:gap-12">
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="relative rounded-2xl overflow-hidden border border-border/50 bg-gradient-to-br from-muted/60 to-muted/10 aspect-[4/3] flex items-center justify-center p-8">
                                ...
                                <div className="absolute top-4 right-4 z-10 flex items-center gap-2 bg-background/90 backdrop-blur-sm border border-border/60 rounded-full px-3 py-1.5 shadow-sm">
                                    {logoUrl ? (
                                        <img src={logoUrl} alt="" width="64" height="16" className="h-4 w-auto object-contain" />
                                    ) : (
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="rounded-2xl border border-border/50 bg-card p-5">
                                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">Teknik Özellikler</p>
                                {(() => {
                                    const values: Record<string, string | undefined> = {
                                        islemci: product.islemci,
                                        ekranKarti: product.ekranKarti,
                                        ram: product.ram,
                                        depolama: product.ssd ?? product.depolama,
                                        anakart: product.anakart,
                                        kasa: product.kasa,
                                        psu: product.psu,
                                        sogutucu: product.sogutucu,
                                    }
                                    return SPEC_STYLES.map(s => (
                                        <SpecRow
                                            key={s.key}
                                        ...
                                        >
                                            ...
                                            <div className="p-3">
                                                <div className="flex items-center gap-1.5 mb-1.5">
                                                    {sLogo ? (
                                                        <img src={sLogo} alt="" width="56" height="14" className="h-3.5 w-auto object-contain" />
                                                    ) : (
