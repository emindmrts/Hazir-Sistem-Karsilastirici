...
import { SPEC_BY_KEY } from "@/lib/spec-styles"
...
/** Ortak ikon + renk kaynağını kullanarak bileşen ikonu basar. */
function SpecIcon({ specKey, className }: { specKey: string; className?: string }) {
    const s = SPEC_BY_KEY[specKey]
    if (!s) return null
    return <s.Icon className={`${className ?? ""} ${s.text}`} />
}
            ...
            <div className="flex sm:hidden">
                ...
                <div className="relative bg-gradient-to-br from-muted/60 to-muted/20 flex items-center justify-center w-28 shrink-0 overflow-hidden">
                    ...
                    <img
                        ...
                        width="400"
                        height="300"
                        sizes="(min-width: 1280px) 320px, (min-width: 1024px) 280px, (min-width: 640px) 200px, 112px"
                ...
                <div className="flex flex-col flex-1 min-w-0 p-3 gap-2 justify-between">
                    <h3 className="font-semibold text-xs leading-snug line-clamp-2 group-hover:text-primary transition-colors">
                        <a href={`/sistem/${slug}`} onClick={(e) => { e.preventDefault(); goToDetail() }} className="hover:underline underline-offset-2">
                            {displayName}
                        </a>
                    </h3>
                    <div className="space-y-0.5 text-[10px] text-muted-foreground">
                        {product.islemci && (
                            <div className="flex items-center gap-1.5">
                                <SpecIcon specKey="islemci" className="w-3 h-3 shrink-0" />
                                <span className="truncate line-clamp-1">{product.islemci}</span>
                            </div>
                        )}
                        {product.ekranKarti && (
                            <div className="flex items-center gap-1.5">
                                <SpecIcon specKey="ekranKarti" className="w-3 h-3 shrink-0" />
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-1 mt-0.5">
                            {product.ram && (
                                <div className="flex items-center gap-1">
                                    <SpecIcon specKey="ram" className="w-3 h-3 shrink-0" />
                                    <span className="whitespace-nowrap">{product.ram}</span>
                                </div>
                            )}
                            {(product.ssd ?? product.depolama) && (
                                <div className="flex items-center gap-1">
                                    <SpecIcon specKey="depolama" className="w-3 h-3 shrink-0" />
                                    <span className="whitespace-nowrap">{product.ssd ?? product.depolama}</span>
                                </div>
                            )}
                            {product.psu && (
                                <div className="flex items-center gap-1">
                                    <SpecIcon specKey="psu" className="w-3 h-3 shrink-0" />
                    ...
                    <div className="space-y-1.5 text-xs text-muted-foreground">
                        {product.islemci && (
                            <div className="flex items-center gap-2">
                                <SpecIcon specKey="islemci" className="w-3.5 h-3.5 shrink-0" />
                                <span className="truncate">{product.islemci}</span>
                            </div>
                        )}
                        {product.ekranKarti && (
                            <div className="flex items-center gap-2">
                                <SpecIcon specKey="ekranKarti" className="w-3.5 h-3.5 shrink-0" />
                                <span className="truncate">{product.ekranKarti}</span>
                            </div>
                        )}
                        {product.anakart && (
                            <div className="flex items-center gap-2">
                                <SpecIcon specKey="anakart" className="w-3.5 h-3.5 shrink-0" />
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 pt-0.5">
                            ...
                            {product.kasa && (
                                <div className="flex items-center gap-1.5 min-w-0">
                                    <SpecIcon specKey="kasa" className="w-3 h-3 shrink-0" />
