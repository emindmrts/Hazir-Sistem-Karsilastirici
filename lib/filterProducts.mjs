...
import { matchCpuSeries, matchGpuBrand, groupByConfig } from "./productIndex.mjs";
...
export function filterProducts(products, f = {}) {
  ...
  // ── Group by hardware config (merge same product, different names) ─────────
  if (f.groupByConfig) {
    const groups = groupByConfig(data);
    groups.sort((a, b) => (sortOrder === "highToLow" ? b.minPrice - a.minPrice : a.minPrice - b.minPrice));
