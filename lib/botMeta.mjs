...
/** Social crawler shell for blog articles (/blog/:slug). */
export function blogBotHtml(article) {
  const blogUrl = `${SITE_URL}/blog`;
  const canonical = `${blogUrl}/${article.slug}`;
  const title = `${article.title} | PcKarşılaştır.com`;
  const desc = article.description || `${article.title} — PcKarşılaştır.com blog yazısı.`;
...
<h1>${t}</h1>
<p>${d}</p>
<p><a href="${esc(canonical)}">Yazının tamamını PcKarşılaştır.com'da oku</a></p>
</body>
</html>
`;
}
